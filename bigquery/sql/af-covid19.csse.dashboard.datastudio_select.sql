SELECT
    * EXCEPT(rn)
    , AVG(new_confirmed) OVER (PARTITION BY Country_Region ORDER BY rn ASC RANGE BETWEEN 12 PRECEDING AND CURRENT ROW) moving_average_new_confirmed
...
FROM (
    SELECT
        * EXCEPT(yesterday_confirmed, yesterday_deaths, yesterday_recovered)
